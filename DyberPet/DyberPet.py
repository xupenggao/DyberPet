...
class PetWidget(QWidget):
    ...
    def _setup_active_window_tracking(self):
        if settings.walk_on_active_window and platform in ['win32', 'darwin']:
            self.active_window_miss_count = 0
            self._schedule_active_window_excursion(initial=True)
        else:
            self._stop_active_window_timers()
            if self.active_window_excursion:
                self._finish_active_window_excursion(reschedule=False)
            else:
                self.active_window_surface = None
                self._sync_floor_pos()
                self._snap_to_floor()

    def walk_on_window_changed(self):
        self._setup_active_window_tracking()
    ...
    def _start_active_window_excursion(self):
        ...
        self.active_window_home_state = {
            ...
            'screen': settings.current_screen,
        }
        ...
        self.active_window_walk_frame = 0
        self.active_window_act_frame = 0
        self._set_active_window_phase('walk')
    ...
    def _set_active_window_phase(self, mode=None):
        ...
        self.active_window_act_frame = 0
        self.active_window_walk_frame = 0
        self.active_window_phase_timer.restart()
        if mode == 'walk':
    ...
    def _finish_active_window_excursion(self, reschedule=True):
        ...
        if home_state:
            home_screen = home_state.get('screen')
            if home_screen is not None:
                self.switch_screen(home_screen)
            self.move(home_state['pos'])
    ...
    def _active_window_walk_step(self):
        if not self.active_window_excursion or self.active_window_surface is None:
            self._finish_active_window_excursion()
            return

        if self.active_window_phase_timer.isValid() and \
                self.active_window_phase_timer.elapsed() >= self.active_window_phase_duration:
            self._set_active_window_phase()

        if self.active_window_mode == 'idle':
            self._active_window_idle_step()
            return
    ...
    def _active_window_walk_act(self, direction):
        if direction >= 0:
            candidates = ['right_walk', 'rightwalk', 'walk_right', 'right', 'default']
        else:
            candidates = ['left_walk', 'leftwalk', 'walk_left', 'left', 'default']
        for act_name in candidates:
            if act_name in self.pet_conf.act_dict:
                return self.pet_conf.act_dict[act_name]
        return self.pet_conf.default

    def _active_window_idle_act(self):
        if hasattr(self, 'active_window_idle_current_act') and self.active_window_act_frame > 0:
            return self.active_window_idle_current_act
        candidates = [
            'sit', 'sitting', 'sit_down',
            'lie', 'lying', 'lay', 'lay_down',
    ...
    def _poll_active_window_surface(self):
        ...
        surface = self.active_window_tracker.get_surface()
        if surface is None:
            self.active_window_miss_count += 1
            if self.active_window_miss_count >= 8 and self.active_window_surface is not None:
                self._finish_active_window_excursion()
            return
    ...
    def _active_surface_enabled(self):
        return self.active_window_excursion and self.active_window_surface is not None

    def _movement_limits(self):
        screen_left = self.current_screen.topLeft().x()
        screen_top = self.current_screen.topLeft().y()
        screen_right = screen_left + self.screen_width
        floor_pos = screen_top + self.screen_height - self.height()
    ...
    def _sync_floor_pos(self):
        self.floor_pos = self._movement_limits()[3]

    def _snap_to_floor(self):
        if settings.onfloor != 1 or settings.draging:
            return
        new_x, new_y = self.limit_in_screen(
            self.pos().x(),
            self.floor_pos + settings.current_anchor[1],
            on_action=True
        )
    ...
    def reset_size(self, setImg=True):
        ...
        self._sync_floor_pos()
        if settings.set_fall:
            y = self.floor_pos+settings.current_anchor[1]
        else:
    ...
    def _legacy_limit_in_screen(self, new_x, new_y, on_action=False):
        # 超出当前屏幕左边界
    ...
    def limit_in_screen(self, new_x, new_y, on_action=False):
        left_limit, right_limit, top_limit, floor_pos = self._movement_limits()
        self.floor_pos = floor_pos

        if new_x+self.width()//2 < left_limit:
            new_x = left_limit-self.width()//2
            if not on_action:
                settings.dragspeedx = -settings.dragspeedx * settings.SPEED_DECAY
                settings.fall_right = not settings.fall_right

        elif new_x+self.width()//2 > right_limit:
            new_x = right_limit-self.width()//2
            if not on_action:
                settings.dragspeedx = -settings.dragspeedx * settings.SPEED_DECAY
                settings.fall_right = not settings.fall_right

        if new_y+self.height()-self.label.height()//2 < top_limit:
            new_y = top_limit + self.label.height()//2 - self.height()
            if not on_action:
                settings.dragspeedy = abs(settings.dragspeedy) * settings.SPEED_DECAY

        elif new_y > floor_pos+settings.current_anchor[1]:
            new_y = floor_pos+settings.current_anchor[1]
